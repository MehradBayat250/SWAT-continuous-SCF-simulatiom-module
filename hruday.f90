          ...
          if (sol_nly(ihru) .ge. 2) then
              do k = 2, 11
                ...
                do ly = 2, sol_nly(ihru)
                    ...
                    elseif ((sol_z(ly-1,ihru) .gt. sum_depth(k-1) .and. sol_z(ly,ihru) .gt. sum_depth(k)).or. (sol_z(ly-1,ihru) .ge. sum_depth(k-1) .and. sol_z(ly,ihru) .gt. sum_depth(k)).or. (sol_z(ly-1,ihru) .gt. sum_depth(k-1) .and. sol_z(ly,ihru) .ge. sum_depth(k))) then
                    ...
                    elseif ((sol_z(ly-1,ihru) .lt. sum_depth(k-1) .and. sol_z(ly,ihru) .lt. sum_depth(k)).or. (sol_z(ly-1,ihru) .le. sum_depth(k-1) .and. sol_z(ly,ihru) .lt. sum_depth(k)).or. (sol_z(ly-1,ihru) .lt. sum_depth(k-1) .and. sol_z(ly,ihru) .le. sum_depth(k)))then
                    ...
                    elseif ((sol_z(ly-1,ihru) .lt. sum_depth(k-1) .and. sol_z(ly,ihru) .gt. sum_depth(k)).or. (sol_z(ly-1,ihru) .le. sum_depth(k-1) .and. sol_z(ly,ihru) .gt. sum_depth(k)).or. (sol_z(ly-1,ihru) .lt. sum_depth(k-1) .and. sol_z(ly,ihru) .ge. sum_depth(k)))then 
...
!!    phos due to crack flow (tvap)
      ...
      if (iscen == 1 .and. isproj == 0) then
        if (icalen == 0) write (28,1001) cropname, j, subnum(j),hruno(j), sb, nmgt(j), iida, hru_km(j),(pdvs(ii), ii = 1, itots)
        if (icalen == 1) write (28,1002) cropname, j, subnum(j),hruno(j), sb, nmgt(j), i_mo, icl(iida), iyr, hru_km(j),(pdvs(ii), ii = 1, itots)
1002  format (a4,i5,1x,a5,a4,i5,1x,i4,1x,i2,1x,i2,1x,i4,1x,e10.5,66f10.3,1x,e10.5,1x,e10.5,8e10.3,3f10.3)
...
!!    added for binary files 3/25/09 gsm line below and write (33333
        ...
        write (21,1000) cropname, j, subnum(j), hruno(j), sb,nmgt(j), iida, hru_km(j), (pdvs(ii), ii = 1, itots)
        else if (iscen == 1 .and. isproj == 2) then
        if(icalen == 0)write (28,1000) cropname, j, subnum(j), hruno(j),sb, nmgt(j), iida, hru_km(j), (pdvs(ii), ii = 1, itots), iyr
        if(icalen == 1)write (28,1003) cropname, j, subnum(j), hruno(j),sb, nmgt(j), i_mo, icl(iida), iyr, hru_km(j),(pdvs(ii), ii = 1, itots), iyr
1003  format(a4,i5,1x,a5,a4,i5,1x,i4,1x,i2,1x,i2,1x,i4,1x,e10.5,66f10.3,1x,e10.5,1x,e10.5,8e10.3,f10.3,1x,i4)
        end if
      else
        if (iscen == 1 .and. isproj == 0) then
        if(icalen == 0)write (28,1000) cropname, j, subnum(j), hruno(j),sb,nmgt(j), iida, hru_km(j), (pdvas(ii), ii = 1, mhruo)
        if(icalen == 1)write (28,1003) cropname, j, subnum(j), hruno(j),sb,nmgt(j), i_mo, icl(iida), iyr, hru_km(j),(pdvas(ii), ii = 1, mhruo)
!!    added for binary files 3/25/09 gsm line below and write (33333
	    if (ia_b == 1) then
             write (33333)  j, hrugis(j), sb,nmgt(j), iida, hru_km(j), (pdvas(ii), ii = 1, mhruo)
	    endif

        else if (isproj == 1) then
        write (21,1000) cropname, j, subnum(j), hruno(j), sb,nmgt(j), iida, hru_km(j), (pdvas(ii), ii = 1, mhruo)
        else if (iscen == 1 .and. isproj == 2) then
        if(icalen == 0)write (28,1000) cropname, j, subnum(j), hruno(j),sb,nmgt(j), iida, hru_km(j), (pdvas(ii), ii = 1, mhruo), iyr
         if(icalen == 1)write(28,1000) cropname, j, subnum(j), hruno(j),sb,nmgt(j), i_mo, icl(iida), iyr, hru_km(j),(pdvas(ii), ii = 1, mhruo), iyr
        end if
      end if
      ...
      if (cswat == 2) then
          ...
          write (98,9000) iyr, i, k, j, sol_mass,sol_cmass,sol_nmass,sol_LS(k,j),sol_LM(k,j),sol_LSC(k,j),sol_LMC(k,j),sol_HSC(k,j),sol_HPC(k,j),sol_BMC(k,j),sol_LSN(k,j),sol_LMN(k,j),sol_HPN(k,j),sol_HSN(k,j),sol_BMN(k,j),sol_no3(k,j),sol_fop(k,j),sol_orgp(k,j),sol_solp(k,j)   
          ...
          write (1001,9001) iyr, i, j, rsdc_d(j), sedc_d(j), percc_d(j),latc_d(j),emitc_d(j), grainc_d(j), surfqc_d(j), stoverc_d(j), NPPC_d(j), foc_d(j),rspc_d(j),tot_mass,tot_cmass,tot_nmass,tot_LSC,tot_LMC,tot_HSC,tot_HPC,tot_BMC,bio_ms(j)*0.42, rwt(j), tot_no3_nh3,wdntl,etday,tillage_factor(j), (soilwater(ii), ii = 1, 11), (wfsc(ii), ii = 1, 11)     
          end if  
      end if
      !!add by zhang
      !!output carbon related variables
      ...
      !! M.B_start Modified part
       write (1029,9999) rooze, j, sno_hru(j)
       write(1045,9992) rooze, j, snocov_me(j)
9992   format(i3,1x,i4,1x,e11.5)       
9999   format(i3,1x,i4,1x,f30.8)     
